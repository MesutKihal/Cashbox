...
plt.style.use('bmh')
conn = sql.connect('assets/db/store.db')
cur = conn.cursor()
...
###################################### Cash Register Page #############################################################
        ...
        def search_bar_text_changed(self):
            ...
            cur.execute("SELECT * FROM products WHERE title = ?", (text,))
            results = cur.fetchall()
            print(results)
        self.__search_btn = QPushButton("Search")
        self.__search_btn.setIcon(QIcon("assets\\img\\search.png"))
        self.__search_btn.clicked.connect(search_bar_text_changed)
        ...
        self.__products_table.setColumnCount(3)
        self.__products_table.setRowCount(20)
        self.__products_table.setAlternatingRowColors(True)
        self.__products_table.setHorizontalHeaderLabels(["Title", "Quantity", "Price"])
        self.__products_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        with shelve.open('bills') as db:
            for row in range(len(db['bill0'])):
                self.__products_table.setItem(row, 0, QTableWidgetItem(db['bill0'][row].get('title')))
                self.__products_table.setItem(row, 1, QTableWidgetItem(str(db['bill0'][row].get('quantity'))))
                self.__products_table.setItem(row, 2, QTableWidgetItem(str(db['bill0'][row].get('price'))))
        ...
        self.__bill_list.addItem(QListWidgetItem("PRODUCT TITLE \t| QUANTITY \t| PRICE"))
...
########################################### Suppliers Page ###########################################################
        ...
        self.__sup_prod_search_bar.setPlaceholderText("Type the name of the supplier you want to select")
        ...
        # Save Button (SUP)
        self.__sup_save = QPushButton("Save")
        self.__sup_save.setIcon(QIcon("assets\\img\\diskette.png"))
        self.__sup_save.setIconSize(QSize(30, 30))
        self.__suppliers_layout.addWidget(self.__sup_save, 3, 4)
        
        # Select Button (SUP)
        self.__sup_save = QPushButton("Select")
        self.__sup_save.setIcon(QIcon("assets\\img\\click.png"))
        self.__sup_save.setIconSize(QSize(30, 30))
        self.__suppliers_layout.addWidget(self.__sup_save, 3, 5)
        ...
        # Bill Generate (SUP)
        self.__sup_bill_generate = QPushButton("Generate Bill")
        self.__sup_bill_generate.setIcon(QIcon("assets\\img\\invoice.png"))
        self.__sup_bill_generate.setIconSize(QSize(30, 30))
        self.__suppliers_layout.addWidget(self.__sup_bill_generate, 4, 3)
        
        # Bill Undo (SUP)
        self.__sup_bill_undo = QPushButton("Undo")
        self.__sup_bill_undo.setIcon(QIcon("assets\\img\\undo.png"))
        self.__sup_bill_undo.setIconSize(QSize(30, 30))
        self.__suppliers_layout.addWidget(self.__sup_bill_undo, 4, 4)
        
        # Bill Save (SUP)
        self.__sup_bill_save = QPushButton("Save Bill")
        self.__sup_bill_save.setIcon(QIcon("assets\\img\\diskette.png"))
        self.__sup_bill_save.setIconSize(QSize(30, 30))
        self.__suppliers_layout.addWidget(self.__sup_bill_save, 4, 5)
######################################################################################################################
########################################### Customers Page ###########################################################
        ...
        # Search Bar (CUS)
        self.__customer_search_bar = QLineEdit()
        self.__customer_search_bar.setPlaceholderText("Type the name of the customer you want to select")
        self.__customers_layout.addWidget(self.__customer_search_bar, 1, 0, 1, 6)
        
        # Search Button (CUS)
        self.__customer_search_btn = QPushButton("Search")
        self.__customer_search_btn.setIcon(QIcon("assets\\img\\search.png"))
        self.__customer_search_btn.setIconSize(QSize(30, 30))
        self.__customers_layout.addWidget(self.__customer_search_btn, 1, 6)

        # Customer Table
        self.__customer_table = QTableWidget()
        self.__customer_table.setColumnCount(4)
        self.__customer_table.setRowCount(20)
        self.__customer_table.setHorizontalHeaderLabels(["ID", "Name", "Phone N°", "Email"])
        self.__customer_table.setAlternatingRowColors(True)
        self.__customer_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.__customers_layout.addWidget(self.__customer_table, 2, 0, 3, 7)

        # Add Customer
        self.__add_customer_button = QPushButton("Add new customer")
        self.__add_customer_button.setIcon(QIcon("assets\\img\\add.png"))
        self.__add_customer_button.setIconSize(QSize(30, 40))
        self.__customers_layout.addWidget(self.__add_customer_button, 5, 3)
 
        # Csutomer Card
        self.__customer_card = QFrame()
        self.__cus_card_layout = QVBoxLayout(self.__customer_card)
        self.__customers_layout.addWidget(self.__customer_card, 1, 8, 5, 1)
        
        
        # Customer Image
        self.__customer_image = QPixmap("assets\\img\\noimage.png")
        self.__customer_image = self.__product_image.scaled(300, 300, Qt.KeepAspectRatio)
        self.__cus_img_container = QLabel()
        self.__cus_img_container.setPixmap(self.__customer_image)
        self.__cus_card_layout.addWidget(self.__cus_img_container)
        
        # Customer ID
        self.__customer_id = QLabel("Customer ID")
        self.__customer_id.setFont(QFont("Helvetica", 14))
        self.__customer_id.setStyleSheet("background: #fff;border-radius: none;")
        self.__cus_card_layout.addWidget(self.__customer_id)
        
        # Customer Name
        self.__customer_name = QLabel("Customer Name")
        self.__customer_name.setFont(QFont("Helvetica", 14))
        self.__customer_name.setStyleSheet("background: #ddd;border-radius: none;")
        self.__cus_card_layout.addWidget(self.__customer_name)
        
        # Customer Phone
        self.__customer_phone = QLabel("Customer Phone N°")
        self.__customer_phone.setFont(QFont("Helvetica", 14))
        self.__customer_phone.setStyleSheet("background: #fff;border-radius: none;")
        self.__cus_card_layout.addWidget(self.__customer_phone)
        
        # Customer Email
        self.__customer_email = QLabel("Customer Email")
        self.__customer_email.setFont(QFont("Helvetica", 14))
        self.__customer_email.setStyleSheet("background: #ddd;border-radius: none;")
        self.__cus_card_layout.addWidget(self.__customer_email)
        
        
        # Customer Edit 
        self.__edit_customer_button = QPushButton("Edit Customer")
        self.__edit_customer_button.setIcon(QIcon("assets\\img\\edit.png"))
        self.__edit_customer_button.setIconSize(QSize(30, 40))
        self.__cus_card_layout.addWidget(self.__edit_customer_button)
